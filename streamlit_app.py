...
def render_summary_view(df: pd.DataFrame, amount_column: Optional[str]) -> None:
    ...
    top_desc_df = top_desc_series.reset_index()
    top_desc_df.columns = ["DESCRIPTION", "Total"]
    prev_visibility = st.session_state.get("top_desc_visibility")
    top_desc_df["Show line"] = top_desc_df["DESCRIPTION"].apply(
        lambda desc: prev_visibility.get(desc, True) if isinstance(prev_visibility, dict) else True
    )
    edited_top_desc = st.data_editor(
        top_desc_df,
        hide_index=True,
        column_config={
            "Show line": st.column_config.CheckboxColumn("Show line", default=True),
            "Total": st.column_config.NumberColumn("Total", format="$%.0f"),
        },
        key="top_desc_editor",
    )
    visible_descriptions = set(
        edited_top_desc.loc[edited_top_desc["Show line"], "DESCRIPTION"].tolist()
    )
    st.session_state["top_desc_visibility"] = dict(
        zip(edited_top_desc["DESCRIPTION"], edited_top_desc["Show line"])
    )
    ...
    if line_source.empty:
        ...
        eligible = set(top_descriptions)
        active_descriptions = eligible if not visible_descriptions else eligible & visible_descriptions
        line_source = line_source[line_source["DESCRIPTION"].isin(active_descriptions)]
        if line_source.empty:
            ...
            pivot = (
                line_data.pivot(index=x_field, columns="DESCRIPTION", values="_ROW_VALUE")
                .fillna(0)
            )
            pivot = pivot.reindex(order_df[x_field].tolist())
            st.line_chart(pivot)
